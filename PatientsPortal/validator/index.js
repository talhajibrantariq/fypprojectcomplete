...
exports.patientSignUpValidation= (req, res, next)=>{
    ...
    req.check("email",).matches(/.+\@.+\..+/).withMessage("Email must contain @").isLength({
        min:4,
        max:3000
    })
    //check for password
    req.check("password","Password is required").notEmpty();
    req.check("password").isLength({min:6}).withMessage("Password must contain atleast 6 character").matches(/\d/).withMessage("Password must contain a number");
    //check for errors
    const errors = req.validationErrors();
    if(errors){
        const firstError = errors.map(error =>error.msg)[0];
